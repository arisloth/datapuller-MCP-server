...
| Path | Layer | Role |
|------|-------|------|
| `streams/` | I/O (push) | WebSocket adapters — `alpaca` (equity trades + NBBO quotes), `binance` (spot aggTrade), `bybit` (perp + spot publicTrade), `hyperliquid` (spot trades) over a shared `base` client (auto-reconnect with exponential backoff, resubscribe-on-reconnect, idle watchdog, venue keepalive). `manager` owns a daemon-thread event loop; subscriptions are lazy (first tool call), budgeted, and LRU-evicted (Alpaca IEX allows 1 connection / ~30 symbols). |
...
|------|-----------|---------|
| `get_klines` | `symbol`, `interval="1h"`, `limit=50` | OHLCV candles + % change over the window. Crypto is served by Binance (spot → futures), falling back to **Bybit / Coinbase / Hyperliquid spot** for symbols Binance doesn't list (applies to every OHLCV-based tool below) |
| `get_orderbook` | `symbol`, `exchange="binance"` (`binance`/`bybit`/`hyperliquid`/`coinbase`; ignored for equities) | Crypto: best bid/ask, spread, 5/10/20-level bid-vs-ask imbalance + pressure. Equities: **live NBBO top-of-book** (bid/ask + sizes, spread, quote age) from the Alpaca stream |
| `get_futures_context` | `symbol` | funding rate **+ annualized APR, percentile & extreme flag**, next funding, mark/index price, **perp basis** (contango/backwardation), open interest + 5h trend, **OI/price quadrant** (long build-up / short build-up / short-covering / long-liquidation) with an **`oi_exhaustion` watch** (long liquidation → contrarian-bullish capitulation read; short covering → contrarian-bearish weak-rally read), and the long/short account ratio **demoted to an extreme-only contrarian flag** — extremes fire on absolute gates **or ≥90th/≤10th percentile vs its own 100h history** (mid-range is labeled noise) |
...
| `get_confluence` | `symbol`, `interval="1h"`, `limit=300` | **Signal-alignment scorecard** — the counterweight to per-tool caveats. Collects the stack's directional reads (regime, EMA stack, CVD, taker over the last 20 bars, VWAP, **stretch: RSI(14) + VWAP σ-distance**, OI quadrant, **OI exhaustion**, funding & L/S contrarian extremes) and scores them by **independent dimension, not raw count** — the price-path reads (regime/EMA/VWAP) share one cluster, the flow reads (CVD/taker) another, and the positioning reads (OI quadrant/exhaustion, funding & L/S extremes) a third, so neither a pure trend sweep nor a pile of correlated crowding flags can self-align. Verdicts: **aligned** (≥3 dimensions agree, none oppose — the methodology's green light), **aligned_extended** (agreement but price stretched — continuation on a pullback, never a chase), **leaning** / **leaning_extended** (majority — reduced size), **mixed** (genuine disagreement — no edge), **no_signal**. Names which reads agree/oppose; squeeze + ATR as context |
| `get_indicators` | `symbol`, `interval="1h"`, `limit=60` | OBV, **CVD (+trend & swing-anchored price divergence** — compares the last two price swing lows/highs against CVD at the same bars, so it can fire at an actual turning point; slope fallback when the window has no swings**)**, **taker buy/sell ratio**, volume ratio (last closed bar vs 20-bar average), ADX(14) with +DI/-DI framed as a **regime gate** (DI actionable only when ADX≥25), **TTM squeeze + Bollinger width**, **candlestick patterns** (confirmation-only), Fibonacci retracements |
| `get_patterns` | `symbol`, `interval="1h"`, `limit=192` | Candlestick patterns on the latest bar (hammer, shooting star, doji, marubozu, engulfing, inside bar) **scored as confirmation only** — each gets a verdict (confirmed/**reversal_watch**/weak/mixed/conflicting/unconfirmed/neutral) from CVD + taker flow + whether it sits at a volume-profile level. For crypto the **live tape's ladder shape feeds the verdict**: a reversal pattern against the (lagging) window flow with the tape **flipping** scores `reversal_watch` — a genuine early-reversal read instead of an automatic "conflicting". Never a standalone signal |
| `get_cvd` | `symbol`, `interval="15m"`, `limit=96` | Crypto: Cumulative Volume Delta on **perp + spot** with trend, taker ratio, CVD-vs-price divergence, and the **spot-vs-perp** conviction read (perp-led = fragile/leverage; spot-led = higher-conviction), plus `live` **trade-level flow from the WebSocket tape** (true aggressor side per print; perp via Bybit, spot venue-routed: Binance → Bybit spot → Hyperliquid spot for Binance-unlisted coins) as a **1m/5m/15m window ladder with a shape label** (accelerating / steady / fading / flipping — building aggression vs stale burst vs absorption; shape gated until the tape spans the 15m window). Equities: **the same live ladder from the Alpaca stream, each print classified via Lee–Ready against the prevailing NBBO** (bars carry no taker side — stream-only). First call subscribes and warms in seconds |
...
| `get_volume_breakdown` | `symbol` | 24h USD volume: Binance / Coinbase / Bybit / Hyperliquid spot + cross-exchange aggregate (via CoinGecko) with shares, **explicit cross-venue perp volume (Binance / Bybit / Hyperliquid)**, **perp/spot ratio + leverage-led vs spot-led read**, `thin` flag — US/institutional divergence on majors, true total on thin alts |
...
- The spot trade tape is **venue-routed per symbol**: Binance, falling back to Bybit spot,
  then Hyperliquid spot for coins Binance doesn't list (probed over REST; successful routes
  cached, failed probes retried) — so Binance-unlisted coins still get a true trade-level
  spot CVD read.
